    ...
    prompts: [
      {
        type: "input",
        name: "name",
        message: "What is the name of the package? (You can skip the `@brand-map/` prefix)",
      },
      {
        type: "input",
        name: "deps",
    ...
    actions: [
      (answers) => {
        if ("name" in answers && typeof answers.name === "string") {
          if (answers.name.startsWith("@brand-map/")) {
            answers.name = answers.name.replace("@brand-map/", "");
          }
        }
